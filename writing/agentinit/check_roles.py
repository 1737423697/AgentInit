...
2. According to the problem and existing expert roles, you should check the selected expert roles.
...
3. According to the problem and existing expert roles, you should check the new expert roles that you have created.
...
3.2. You should include the following information for each new expert role: a name, a detailed description of their area of expertise, some suggestions for executing the task, and a prompt template for calling them.
...
3. DO NOT ask any questions to the user or human. 
-----
'''
...
class CheckRoles(Action):
    def __init__(self, name="Check Roles", context=None, llm_name=None):
    ...
    async def run(self, context, history=''):
        ...
        prompt = PROMPT_TEMPLATE.format(question=question, history=history, existing_roles=ROLES_LIST, created_roles=created_roles, selected_roles=selected_roles, format_example=FORMAT_EXAMPLE)
