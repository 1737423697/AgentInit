...
class Tokenizer(Singleton):
    def __init__(self, model):
        if "Llama" or "llama" in model:
            self.tokenizer = AutoTokenizer.from_pretrained('', use_fast=True)
        else:
            self.tokenizer = AutoTokenizer.from_pretrained('', use_fast=True)#Qwen Tokenizer
        self.value = ""
