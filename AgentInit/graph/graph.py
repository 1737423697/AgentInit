...
class Graph(ABC):
    ...
    def __init__(self, 
        ...
        self.fixed_spatial_masks = fixed_spatial_masks if isinstance(fixed_spatial_masks, torch.Tensor) else torch.tensor(fixed_spatial_masks)
        self.fixed_temporal_masks = fixed_temporal_masks if isinstance(fixed_temporal_masks, torch.Tensor) else torch.tensor(fixed_temporal_masks)
        ...
        fixed_spatial_masks = fixed_spatial_masks.view(-1) if isinstance(fixed_spatial_masks, torch.Tensor) else torch.tensor(fixed_spatial_masks).view(-1)
        fixed_temporal_masks = fixed_temporal_masks.view(-1) if isinstance(fixed_temporal_masks, torch.Tensor) else torch.tensor(fixed_temporal_masks).view(-1)
        ...
        self.initial_spatial_probability = initial_spatial_probability
        self.initial_temporal_probability = initial_temporal_probability
        ...
        if dec:
            ...
            self.decision_logits = torch.ones(len(agent_names)) * torch.log(torch.tensor(1.0))
        ...
        # print("Final spatial logits: ",self.spatial_logits)
        # print("Final temporal logits: ",self.temporal_logits)
        # print("Final spatial masks: ",self.spatial_masks)
        # print("Final temporal masks: ",self.temporal_masks)
    ...
    def init_nodes(self):
        ...
        for agent_name,kwargs in zip(self.agent_names,self.node_kwargs):
            if agent_name in AgentRegistry.registry:
                ...
                if "AutoAgent" in agent_name:
                    kwargs["ROLE_PROMPT"] = self.role_prompt
                    # print(kwargs)
